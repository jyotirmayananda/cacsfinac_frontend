...
export function ITRPlansPopup({ open, onOpenChange }: DialogProps) {
  ...
  return (
    <>
      ...
      <DialogContent className={cn("max-w-[95vw] lg:max-w-5xl xl:max-w-7xl w-full p-4 md:p-6 bg-card rounded-3xl max-h-[90svh] overflow-y-auto overflow-x-hidden border border-cyan-500/20 shadow-[0_0_50px_-12px_rgba(0,180,216,0.3)] backdrop-blur-md")}>
        <DialogHeader className="mb-6 md:mb-10 w-full">
          <div className="flex flex-col items-center justify-center text-center w-full space-y-3">
            <DialogTitle className="text-xl md:text-3xl font-extrabold font-headline uppercase tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-cyan-500 w-full text-center">
              Income Tax Return Plans
            </DialogTitle>
            <DialogDescription className="text-muted-foreground text-sm md:text-base max-w-2xl mx-auto font-medium text-center">
        ...
        <div className="w-[90%] md:w-[85%] mx-auto relative px-2 md:px-6">
          <Carousel opts={{ align: "start", loop: true }} className="w-full pb-4">
            ...
            {itrPlans.map((plan, index) => (
              <CarouselItem key={index} className="pl-3 md:pl-4 basis-[85%] md:basis-1/2">
                <Card className="relative flex flex-col h-full bg-card/80 backdrop-blur-sm border border-border/50 hover:border-cyan-500/50 hover:shadow-[0_8px_30px_rgb(0,180,216,0.15)] transition-all duration-500 rounded-2xl overflow-hidden group hover:-translate-y-1">
                  ...
                  <CardHeader className="pb-4 pt-6 px-5 md:px-6">
                    <CardTitle className="text-lg font-bold text-foreground group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-blue-600 group-hover:to-cyan-500 transition-all">
                      {plan.title}
                    </CardTitle>
                    <div className="mt-3 flex items-end gap-1">
                      <span className="text-2xl md:text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-br from-blue-600 to-cyan-500 tracking-tight pb-1">
                        ₹{plan.price}
                      </span>
                    ...
                    <ul className="space-y-3">
                      {plan.includes.map((item, i) => (
                        <li key={i} className="flex items-start gap-2 text-xs md:text-sm text-muted-foreground font-medium">
                          <CheckCircle2 className="h-4 w-4 text-cyan-500 flex-shrink-0 drop-shadow-sm" />
